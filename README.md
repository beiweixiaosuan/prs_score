...
Odds ratio (OR) + standard error (SE):
...
Run PRS-CS per chromosome (1–22)
Concatenate per-chromosome posterior effect sizes
Use PLINK --score to compute individual-level PRS
...
Place or generate the GWAS summary statistics in PRS-CS format (SNP, A1, A2, and either OR+SE or BETA+P).
Update the paths, TRAIT, GWAS_SST, and N_GWAS in the SLURM script.
Submit the job to the cluster via sbatch.
After completion, use the .profile file from the --out prefix (for example, AN_prs_score.profile) as the trait’s PRS in downstream analyses.
References
